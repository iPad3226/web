    ...
    for(let i in obj.type){
        if(obj.type[i]=="lift"&&P!="NoLift"){
            if(b>obj.xy[i][0]&&a<obj.xy[i][0]+16){
                if(D<=obj.xy[i][1]+(obj.p[i][1]==0?parseInt(obj.p[i][0],16)/2:obj.p[i][1]==2?-parseInt(obj.p[i][0],16)/2:0)&&d>=obj.xy[i][1]){
                    if(obj.p[i][1]==0){
                        moveplayer(0,-parseInt(obj.p[i][0],16)/2,"NoLift")
                    }else if(obj.p[i][1]==1){
                        moveplayer(parseInt(obj.p[i][0],16)/2,0,"NoLift")
                    }else if(obj.p[i][1]==2){
                        moveplayer(0,parseInt(obj.p[i][0],16)/2,"NoLift")
                    }else if(obj.p[i][1]==3){
                        moveplayer(-parseInt(obj.p[i][0],16)/2,0,"NoLift")
